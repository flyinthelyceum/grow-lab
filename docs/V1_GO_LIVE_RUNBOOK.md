...
## Phase B: Sensor Integration — done

The SEN0308 and AS7341 were wired and validated during the April 1 go-live.
Wiring is in `WIRING_&_BUSES.md`; per-sensor bring-up is in git history.
...
There is no `enabled` key on `[lighting]` — an earlier version of this runbook told you to
set one and the loader silently discarded it. The schedule is armed by `on_hour`/`off_hour`.
...
Ensure all sensors enabled, irrigation schedule set, and the `[lighting]` photoperiod set in `config.toml`.
